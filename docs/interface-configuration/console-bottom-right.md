...
Pour se faire il faut exécuter le logiciel en mode {{ "Administrateur" | keyword }} voici comment faire.

Lorsque vous avez positionné un raccourci sur votre bureau pour accéder à {{ "TradingInPython" | keyword }} vous cliquez-droit sur le raccourci -> Propriétés -> Raccourci
...
- **(1)** Clic droit sur le raccourcis puis Propriétés pour accéder aux propriétés du raccourci.
- **(2)** Dans l'onglet Raccourcis -> cliquez sur Avancé
- **(3)** Cochez la case "Exécuter en tant qu'administrateur"

Ainsi, la Console se positionnera automatiquement en bas à droite de votre écran.
