...
- **(1)** Indique simplement que l'action Air Liquide est sélectionnée dans le screener {{ "mon-portefeuille.json" | keyword }}

Vous pouvez cocher ou décocher des stocks pour les ajouter ou les retirer du screener.

Un fois la selection effectuée vous sauver votre screener en venant écraser l'ancienne version.

- {{ "Ouvrir" | keyword }} vous permet de changer de screener.
- {{ "Sauver" | keyword }} vous permet de sauver les modifications que vous avez faites.
- {{ "Supprimer" | keyword }} vous supprimer un fichier de screener.
